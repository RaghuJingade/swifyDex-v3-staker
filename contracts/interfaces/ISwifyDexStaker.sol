...
/// @title SwifyDex Staker Interface
/// @notice Allows staking nonfungible liquidity tokens in exchange for reward tokens
interface ISwifyDexStaker is IERC721Receiver, IMulticall {
    /// @param rewardToken The token being distributed as a reward
    /// @param pool The SwifyDex pool
    ...
    /// @notice The SwifyDex Factory
    function factory() external view returns (ISwifyDexFactory);
    ...
    /// @notice Withdraws a SwifyDex LP token `tokenId` from this contract to the recipient `to`
    ...
    /// @notice Event emitted when a SwifyDex LP token has been staked
    /// @param tokenId The unique identifier of an SwifyDex LP token
    /// @param liquidity The amount of liquidity staked
    /// @param incentiveId The incentive in which the token is staking
    event TokenStaked(uint256 indexed tokenId, bytes32 indexed incentiveId, uint128 liquidity);

    /// @notice Event emitted when a SwifyDex LP token has been unstaked
    /// @param tokenId The unique identifier of an SwifyDex LP token
    /// @param incentiveId The incentive in which the token is staking
